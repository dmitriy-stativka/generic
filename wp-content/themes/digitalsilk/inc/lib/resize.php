...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
    function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
        ...
        if ( empty( $url ) )
            return new WP_Error( 'no_image_url', __( 'No image URL has been entered.','dstheme' ), $url );
        ...
        if ( 'bmp' == $ext ) {
            return new WP_Error( 'bmp_mime_type', __( 'Image is BMP. Please use either JPG or PNG.','dstheme' ), $url );
        }
    ...
    function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
        ...
        if ( !file_exists( $dest_file_name ) ) {
            ...
            if ( !$size )
                return new WP_Error( 'file_path_getimagesize_failed', __( 'Failed to get $file_path information using "@getimagesize".','dstheme'), $file_path );
            ...
            if ( IMAGETYPE_GIF == $orig_type ) {
                if ( !imagegif( $new_image, $dest_file_name ) )
                    return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (GIF)','dstheme' ) );
            }
            elseif ( IMAGETYPE_PNG == $orig_type ) {
                if ( !imagepng( $new_image, $dest_file_name ) )
                    return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (PNG).','dstheme' ) );
            }
            else {
                ...
                if ( !imagejpeg( $new_image, $dest_file_name, apply_filters( 'resize_jpeg_quality', 90 ) ) )
                    return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (JPG).','dstheme' ) );
            ...
            if ( !$new_size )
                return new WP_Error( 'resize_path_getimagesize_failed', __( 'Failed to get $dest_file_name (resized image) info via @getimagesize','dstheme' ), $dest_file_name );
